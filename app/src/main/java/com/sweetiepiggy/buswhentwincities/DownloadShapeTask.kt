...
class DownloadShapeTask(private val mDownloadedListener: OnDownloadedShapeListener,
    ...
    private var mShape: List<LatLng>? = null

    interface OnDownloadedShapeListener {
        fun onDownloadedShape(shapeId: Int, shape: List<LatLng>)
    }
    ...
    private fun downloadShape(shapeId: Int): List<LatLng>?  {
    ...
    private fun parseShape(reader: JsonReader): List<LatLng> {
        val shape: MutableList<Pair<Int, LatLng>> = mutableListOf()
        ...
        while (!isCancelled && reader.hasNext()) {
            ...
            if (shapeId != null && shapePtLat != null && shapePtLon != null && shapePtSequence != null) {
                ...
                shape.add(Pair(shapePtSequence, LatLng(shapePtLat, shapePtLon)))
            }
        }
        reader.endArray()
        dbHelper.close()
